    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user and interaction.user.id == self.author_id:
            ...
            await interaction.response.send_message(
                content="This confirmation dialog is not for you.", ephemeral=True
            )
            return False
        ...
        if self.delete_after and self.message:
            if not self.message.flags.ephemeral:
                ...
                await self.message.edit(
                    view=None, content="This is safe to dismiss now."
                )
...
class DisambiguatorView(discord.ui.View, t.Generic[T]):
...
class EveContext(commands.Context):
    ...
    async def entry_to_code(self, entries: t.Iterable[tuple[str, str]]) -> None:
    ...
    async def indented_entry_to_code(
        self, entries: t.Iterable[tuple[str, str]]
    ) -> None:
        width = max(len(a) for a, b in entries)
        output = ["```"]
        for name, entry in entries:
    ...
    def replied_reference(self) -> t.Optional[discord.MessageReference]:
    ...
    @discord.utils.cached_property
    def replied_message(self) -> t.Optional[discord.Message]:
        ref = self.message.reference
        if ref and isinstance(ref.resolved, discord.Message):
            return ref.resolved
        return None
    ...
    def tick(self, opt: t.Optional[bool], label: t.Optional[str] = None) -> str:
        lookup = {
    ...
    async def show_help(self, command: t.Any = None) -> None:
