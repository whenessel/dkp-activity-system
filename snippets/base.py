...
RESPAWN_RE = re.compile(
    r"\D+\s(\d{2}:\d{2}\*?|--:--)(\s+\d{2,3}%?)?(\s<-track)?", re.MULTILINE
)
...
async def event_template_autocomplete(
    ...
    templates: t.List[EventTemplate] = list(EventTemplate.objects.all().order_by("id"))
    return [
        app_commands.Choice(
        ...
        if (current in template.title) or (current in template.description)
    ]
...
def event_template_embed(template: EventTemplate) -> discord.Embed:
    ...
    embed.add_field(
        name="Тип события", value=f"{template.get_type_display()}", inline=False
    )
    embed.add_field(
        name="Вознаграждение",
        value=f"{template.cost} дкп "
        f"за {template.capacity} / {template.get_unit_display()}",
        inline=False,
    )
    embed.add_field(name="Опоздание", value=f"{template.penalty}% дкп", inline=True)
    embed.add_field(
        name="Наличие варов", value=f"+{template.military}% дкп", inline=True
    )
    embed.add_field(name="Ночь", value=f"+{template.overnight}% дкп", inline=True)
    return embed
...
def event_embed(event: "EventItem") -> discord.Embed:
    ...
    embed.set_author(
        name=f"РЛ: {event.author.display_name}", icon_url=event.author.avatar.url
    )
    ...
    if event.status == EventStatus.FINISHED:
        ...
        economy += (
            f"**Присутствовавшие**: {event.full_reward} дкп\n"
            f"**Опоздавшие**: {event.partial_reward} дкп\n"
        )

    embed.add_field(name="Бухгалтерия", value=economy, inline=False)

    embed.add_field(name="Номер события", value=f"{event.id}", inline=False)
    embed.add_field(
        name="Статус события", value=f"{event.get_status_display()}", inline=True
    )
    embed.add_field(
        name="Время события",
    ...
    embed.add_field(
        name="Наличие варов",
        value=f'**{"Да" if event.is_military else "Нет"}**',
        inline=True,
    )
    embed.add_field(
        name="Ночной",
        value=f'**{"Да" if event.is_overnight else "Нет"}**',
        inline=True,
    )
    ...
    if event.status == EventStatus.FINISHED:
        ...
        full_attended_members = list(
            event.event_attendances.filter(type=AttendanceType.FULL).values_list(
                "member_id", flat=True
            )
        )
        partial_attended_members = list(
            event.event_attendances.filter(type=AttendanceType.PARTIAL).values_list(
                "member_id", flat=True
            )
        )

        full_filtered_members = list(
            filter(lambda member: member.id in full_attended_members, guild_members)
        )
        partial_filtered_members = list(
            filter(lambda member: member.id in partial_attended_members, guild_members)
        )

        full_attended = "\n".join([member.mention for member in full_filtered_members])
        partial_attended = "\n".join(
            [member.mention for member in partial_filtered_members]
        )
        embed.add_field(name="Присутствовавшие", value=f"{full_attended}", inline=True)
        embed.add_field(name="Опоздавшие", value=f"{partial_attended}", inline=True)

    return embed


class MemberReactions(
    EmojiEnumMIxin, EnumProperties, s("emoji"), s("attend_type", case_fold=True)
):
...
class EventTemplateTransformer(app_commands.Transformer):
    async def transform(
        self, interaction: discord.Interaction, value: t.Union[str, int]
    ) -> EventTemplate:
...
class EventItem(Event):
    ...
    def add_member_attendance(
        self, member: discord.Member, type: AttendanceType, force: bool = True
...
class QuantityModal(discord.ui.Modal):
    ...
    def __init__(self, cog: "EventCog", event: EventItem):
        ...
        if self.event.unit == CapacityUnit.TIME:
            self.quantity.label = "Введите количество минут"
        elif self.event.unit == CapacityUnit.THING:
            self.quantity.label = "Введите количество боссов"
        elif self.event.unit == CapacityUnit.VISIT:
            self.quantity.label = "Введите количество посещений"
...
class EventButtonsPersistentView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if not self.cog.is_event_moderator(self.event.guild_id, interaction.user.id):
            await interaction.response.send_message(
                ":face_with_symbols_over_mouth: Убрал руки! Это только для модераторов.",
                ephemeral=True,
            )
            return False
        ...
        if self.event.status in [EventStatus.FINISHED, EventStatus.CANCELED]:
            ...
            await interaction.followup.send(
                f":face_with_spiral_eyes: Упс! Событие уже {self.event.get_status_display()}",
                ephemeral=True,
            )
            return False
    ...
    async def success(
        ...
        await interaction.response.send_modal(
            QuantityModal(cog=self.cog, event=self.event)
        )
...
class EventCog(BaseEventCog):
    ...
    async def context_event_finish(
        ...
        if event.status in [EventStatus.FINISHED, EventStatus.CANCELED]:
            await interaction.response.send_message(
                f":face_with_spiral_eyes: Упс! Событие уже {event.get_status_display()}",
                ephemeral=True,
            )
            return False
    ...
    async def on_raw_reaction_add(
        ...
        if str(payload.emoji) in ModeratorReactions.emojis():
            if not self.is_event_moderator(
                guild_id=payload.guild_id, member_id=member.id
            ):
                await message.remove_reaction(payload.emoji, member)
                return
    ...
    @app_commands.choices(
        type=[
            ...
            for value, label in AttendanceType.choices
        ]
    )
    async def event_mod_member_add(
        self, ctx: GuildEveContext, member: discord.Member, type: str, event: int
    ) -> None:
        try:
            ...
            await ctx.send(
                ...
                f"Событие: {event_message.jump_url}",
    ...
    @checks.event_moderator_only()
    @app_commands.describe(
        member="Участник, которого надо добавить к событию", event="Номер события"
    )
    ...
    async def event_statistic(
        self,
        ctx: GuildEveContext,
        start: str,
        end: str,
        member: t.Optional[discord.Member],
    ) -> None:
    ...
    @checks.event_moderator_only()
    @app_commands.describe(
        template="Шаблон события", description="Описание события или список респов"
    )
...
class AdminEventCog(BaseEventCog):
    ...
    async def event_admin_moderator_show(self, ctx: GuildEveContext) -> None:
        ...
        await ctx.send("Пока не работает :ghost:", reference=ctx.message)
    ...
    @app_commands.choices(
        ...
        unit=[
            app_commands.Choice(name=label, value=value)
            for value, label in CapacityUnit.choices
        ],
    )
    async def event_admin_template_add(
        ...
        capacity: int,
        unit: str,
        penalty: t.Optional[int] = 50,
        military: t.Optional[int] = 25,
        overnight: t.Optional[int] = 25,
    ) -> None:
...
class ModEventCog(commands.Cog):
    ...
